...
from utils.globals import squareSide, buttonRadius, colors, auxDirectory, info_image, width, mute_image, unmute_image
...
# INFO
# This functions renders an in-game help
...
def show_info(screen, scr_width, clock):
    while True:
        ...
        line = other_text.render("2. Choose each player's paddle color at the title screen.", True, WHITE)
        screen.blit(line, (100, 260))

        line = other_text.render("3. To start playing, click on the difficulty level.", True, WHITE)
        screen.blit(line, (100, 300))

        line = other_text.render("4. Each game comprises of three rounds, and the player who wins ", True, WHITE)
        screen.blit(line, (100, 350))
        line = other_text.render("two (or more) rounds is the winner.", True, WHITE)
        ...
        line = other_text.render("or clicking the pause icon on the screen.", True, WHITE)
        ...
        back = other_text.render("BACK", True, BLACK)
...
def air_hockey_start(screen, clock, scr_width, scr_height, mute):
    ...
    sel_p2 = SelBox(2, 3)  # for player's color
    ...
    while True:
        ...
        pygame.draw.rect(screen, WHITE, (x2, y2, 320, 50), 1)

        # box for player2's name
        if player_2_name == "":
            player_2_text = small_text.render("Player 2 Name", True, WHITE)
        else:
            player_2_text = small_text.render(player_2_name, True, WHITE)
                    ...
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.locals.KEYDOWN:
                            if event.unicode.isalpha() and not (len(player_1_name) > 8):
                                ...
                                ret = 1
                                player_1_key = False
                    pygame.draw.rect(screen, WHITE, (x1, y1, 320, 50), 0)
                    if not (player_1_name == ""):
                        player_1_text = small_text.render("{0}{1}".format(player_1_name, blink_ch), True, BLACK)
        ...
        pygame.draw.rect(screen, WHITE, (x1, y1, 320, 50), 1)
        if player_1_name == "":
            player_1_text = small_text.render("Player 1 Name", True, WHITE)
        else:
            player_1_text = small_text.render(player_1_name, True, WHITE)
                    ...
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.locals.KEYDOWN:
                            if event.unicode.isalpha() and not (len(player_2_name) > 8):
                                ...
                                ret = 1
                    pygame.draw.rect(screen, WHITE, (x2, y2, 320, 50), 0)
                    if not (player_2_name == ""):
                        player_2_text = small_text.render("{0}{1}".format(player_2_name, blink_ch), True, BLACK)
